...
### This version has been adapted for use by Sam Wines (slwines@deakin.edu.au)
...
# Set directories----
rm(list=ls())
study<-"PointAddis2018"

data.dir=("/Users/samwines/Desktop/Uni-Honours/02_Data/07_HabitatData/Langlois_Format/2018")
...
setwd(habitat.data)
dir()
hab<-read.delim('AD_2018_Dot Point Measurements.txt',header=T,skip=4,stringsAsFactors=FALSE)

hab<-read.delim('AD_2018_Dot Point Measurements.txt',header=T,skip=4,stringsAsFactors=FALSE)%>%
  setNames(tolower(names(.)))%>%
 # mutate(OpCode= paste(hab$Sample.No, hab$CampaignID, sep="_"))%>%
  select(-c(filename,frame,time..mins.,date,location,camera,latitude,longitude,rugosity,depth,collector,fishing.status,spare,spare.1,x,x.1,radius..,code))
head(hab,2)
...
broad.point.score<-hab%>%
  ...
  mutate(broad=ifelse(broad=="Octocoral/Black","Octocoral.Black",ifelse(broad=="Invertebrate complex","Invertebrate.complex",ifelse(broad=="Open Water","Open.Water",broad))))%>% #correct bad names
  filter(!broad=="")%>%
  filter(!is.na(broad))%>%
...
broad.percent.cover<-broad.point.score%>%
  ...
  select(-Total.Sum)%>%
  mutate(`Biota: Reef`=`Biota: Macroalgae`) #Add in a Reef classification 
...
habitat.point.score<-relief.point.score%>%
  left_join(fov.point.score,by="site..")%>%
  left_join(broad.point.score,by="site..")
# %>%
#   left_join(broad_morphology_type.point.score,by="site..") #use this line if you have morphology_type data
head(habitat.point.score,2)
write.table(habitat.point.score,file=paste("x",study,"R_habitat.point.score.txt",sep = "_"), sep = "\t", row.names = FALSE)


habitat.percent.cover.and.mean.and.sd.of.relief<-relief.mean.and.sd%>%
  left_join(fov.percent.cover,by="site..")%>%
  left_join(broad.percent.cover,by="site..")
# %>%
#   left_join(broad_morphology_type.percent.cover,by="site..") #use this line if you have morphology_type data
head(habitat.percent.cover.and.mean.and.sd.of.relief,2)
write.table(habitat.percent.cover.and.mean.and.sd.of.relief,file=paste("x",study,"R_habitat.percent.cover.and.mean.and.sd.of.relief.txt",sep = "_"), sep = "\t", row.names = FALSE)
...
habitat.plot<-habitat.percent.cover.and.mean.and.sd.of.relief%>%
  ...
  filter(habitat%in%c("mean.relief","sd.relief","Biota: Consolidated","Biota: Macroalgae","Biota: Invertebrate.complex","Biota: Octocoral.Black","Biota: Sponges","Biota: Stony.corals","Biota: Unconsolidated","Biota: Reef"))
tail(habitat.plot)
