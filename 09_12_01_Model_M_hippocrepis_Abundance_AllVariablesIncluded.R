...
# This script is for modelling abundance of Meuschenia hippocrepis (Horseshoe Leatherjacket)
...
TrainingData.5m.M.hippocrepis.Abundance <- Import.5m.Site.Summaries %>%
...
TestData.5m.M.hippocrepis.Abundance <- Import.5m.Site.Summaries %>%
...
TrainingData.5m.M.hippocrepis.Abundance$Year <- factor(TrainingData.5m.M.hippocrepis.Abundance$Year)
TrainingData.5m.M.hippocrepis.Abundance$Status <- factor(TrainingData.5m.M.hippocrepis.Abundance$Status)
TestData.5m.M.hippocrepis.Abundance$Year <- factor(TestData.5m.M.hippocrepis.Abundance$Year)
TestData.5m.M.hippocrepis.Abundance$Status <- factor(TestData.5m.M.hippocrepis.Abundance$Status)


# Make Correlation Matrix

ggcorr(TrainingData.5m.M.hippocrepis.Abundance[, 2:length(TrainingData.5m.M.hippocrepis.Abundance)], method = c("pairwise", "spearman"), geom = "blank", label = TRUE, hjust = 0.8, label_size = 2, size = 3) +
...
pairs(TrainingData.5m.M.hippocrepis.Abundance)

#Check Response Variable

plot(TrainingData.5m.M.hippocrepis.Abundance$M.hippocrepis.Abundance)


### Transformations
# Example: TrainingData.5m.M.hippocrepis.Abundance$sqrt.rugosity_mean_5_clip_01=sqrt(TrainingData.5m.M.hippocrepis.Abundance$rugosity_mean_5_clip_01)



# Models loop

data.5m <- TrainingData.5m.M.hippocrepis.Abundance

vars.5m <- c("M.hippocrepis.Abundance",
             #"Training0.Test1",
             "Year",
             "Status",
             #vrm_neigh3_mean_5_clip_01, 
...
for(i in seq(COMB.5m)){
  ...
  for(j in seq(ncol(tmp.5m))){
    k.5m <- k.5m + 1
    COMB2.5m[[k.5m]] <- formula(paste("M.hippocrepis.Abundance", "~", paste(tmp.5m[,j], collapse=" + ")))
  }
}
...
TestData.5m.M.hippocrepis.Abundance$Predicted.Species.Richness.5m <- predict.gam(ModelsResults.5m[[best.model.5m]], TestData.5m.M.hippocrepis.Abundance)
TestData.5m.M.hippocrepis.Abundance$Difference.Pred.True.5m <- abs((TestData.5m.M.hippocrepis.Abundance$M.hippocrepis.Abundance) - (TestData.5m.M.hippocrepis.Abundance$Predicted.Species.Richness.5m))
Mean.5m.Species.Richness.Predictive.Difference.5m <- mean(TestData.5m.M.hippocrepis.Abundance$Difference.Pred.True.5m)
...
TrainingData.10m.M.hippocrepis.Abundance <- Import.10m.Site.Summaries %>%
  filter(Training0.Test1 == "0") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
TestData.10m.M.hippocrepis.Abundance <- Import.10m.Site.Summaries %>%
  filter(Training0.Test1 == "1") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
ggcorr(TrainingData.10m.M.hippocrepis.Abundance[, 2:length(TrainingData.10m.M.hippocrepis.Abundance)], method = c("pairwise", "spearman"), geom = "blank", label = TRUE, hjust = 0.8, label_size = 2, size = 3) +
  geom_point(size = 7, aes(color = coefficient > 0, alpha = abs(coefficient) > 0.5)) +
  scale_alpha_manual(values = c("TRUE" = 0.25, "FALSE" = 0)) +
  guides(color = FALSE, alpha = FALSE)
...
pairs(TrainingData.10m.M.hippocrepis.Abundance)

#Check Response Variable

plot(TrainingData.10m.M.hippocrepis.Abundance$M.hippocrepis.Abundance)


### Transformations
# Example: TrainingData.10m.M.hippocrepis.Abundance$sqrt.rugosity_mean_10_clip_01=sqrt(TrainingData.10m.M.hippocrepis.Abundance$rugosity_mean_10_clip_01)



# Models loop

data.10m <- TrainingData.10m.M.hippocrepis.Abundance

vars.10m <- c("M.hippocrepis.Abundance",
              #"Training0.Test1",
              "Year",
              "Status",
              #vrm_neigh3_mean_5_clip_01, 
...
for(i in seq(COMB.10m)){
  ...
  for(j in seq(ncol(tmp.10m))){
    k.10m <- k.10m + 1
    COMB2.10m[[k.10m]] <- formula(paste("M.hippocrepis.Abundance", "~", paste(tmp.10m[,j], collapse=" + ")))
  }
}
...
# Predict for test data
TestData.10m.M.hippocrepis.Abundance$Predicted.Species.Richness.10m <- predict.gam(ModelsResults.10m[[best.model.10m]], TestData.10m.M.hippocrepis.Abundance)
TestData.10m.M.hippocrepis.Abundance$Difference.Pred.True.10m <- abs((TestData.10m.M.hippocrepis.Abundance$M.hippocrepis.Abundance) - (TestData.10m.M.hippocrepis.Abundance$Predicted.Species.Richness.10m))
Mean.10m.Species.Richness.Predictive.Difference.10m <- mean(TestData.10m.M.hippocrepis.Abundance$Difference.Pred.True.10m)
...
TrainingData.25m.M.hippocrepis.Abundance <- Import.25m.Site.Summaries %>%
  filter(Training0.Test1 == "0") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
TestData.25m.M.hippocrepis.Abundance <- Import.25m.Site.Summaries %>%
  filter(Training0.Test1 == "1") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
ggcorr(TrainingData.25m.M.hippocrepis.Abundance[, 2:length(TrainingData.25m.M.hippocrepis.Abundance)], geom = "blank", label = TRUE, hjust = 0.8, label_size = 2, size = 3) +
  geom_point(size = 7, aes(color = coefficient > 0, alpha = abs(coefficient) > 0.5)) +
  scale_alpha_manual(values = c("TRUE" = 0.25, "FALSE" = 0)) +
  guides(color = FALSE, alpha = FALSE)
...
pairs(TrainingData.25m.M.hippocrepis.Abundance)

#Check Response Variable

plot(TrainingData.25m.M.hippocrepis.Abundance$M.hippocrepis.Abundance)


### Transformations
# Example: TrainingData.25m.M.hippocrepis.Abundance$sqrt.rugosity_mean_25_clip_01=sqrt(TrainingData.25m.M.hippocrepis.Abundance$rugosity_mean_25_clip_01)



# Models loop

data.25m <- TrainingData.25m.M.hippocrepis.Abundance

vars.25m <- c("M.hippocrepis.Abundance",
              #"Training0.Test1",
              "Year",
              "Status",
              #vrm_neigh3_mean_5_clip_01, 
...
for(i in seq(COMB.25m)){
  ...
  for(j in seq(ncol(tmp.25m))){
    k.25m <- k.25m + 1
    COMB2.25m[[k.25m]] <- formula(paste("M.hippocrepis.Abundance", "~", paste(tmp.25m[,j], collapse=" + ")))
  }
}
...
# Predict for test data
TestData.25m.M.hippocrepis.Abundance$Predicted.Species.Richness.25m <- predict.gam(ModelsResults.25m[[best.model.25m]], TestData.25m.M.hippocrepis.Abundance)
TestData.25m.M.hippocrepis.Abundance$Difference.Pred.True.25m <- abs((TestData.25m.M.hippocrepis.Abundance$M.hippocrepis.Abundance) - (TestData.25m.M.hippocrepis.Abundance$Predicted.Species.Richness.25m))
Mean.25m.Species.Richness.Predictive.Difference.25m <- mean(TestData.25m.M.hippocrepis.Abundance$Difference.Pred.True.25m)
...
TrainingData.50m.M.hippocrepis.Abundance <- Import.50m.Site.Summaries %>%
  filter(Training0.Test1 == "0") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
TestData.50m.M.hippocrepis.Abundance <- Import.50m.Site.Summaries %>%
  filter(Training0.Test1 == "1") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
ggcorr(TrainingData.50m.M.hippocrepis.Abundance[, 2:length(TrainingData.50m.M.hippocrepis.Abundance)], geom = "blank", label = TRUE, hjust = 0.8, label_size = 2, size = 3) +
  geom_point(size = 7, aes(color = coefficient > 0, alpha = abs(coefficient) > 0.5)) +
  scale_alpha_manual(values = c("TRUE" = 0.25, "FALSE" = 0)) +
  guides(color = FALSE, alpha = FALSE)
...
pairs(TrainingData.50m.M.hippocrepis.Abundance)

#Check Response Variable

plot(TrainingData.50m.M.hippocrepis.Abundance$M.hippocrepis.Abundance)


### Transformations
# Example: TrainingData.50m.M.hippocrepis.Abundance$sqrt.rugosity_mean_50_clip_01=sqrt(TrainingData.50m.M.hippocrepis.Abundance$rugosity_mean_50_clip_01)



# Models loop

data.50m <- TrainingData.50m.M.hippocrepis.Abundance

vars.50m <- c("M.hippocrepis.Abundance",
              #"Training0.Test1",
              "Year",
              "Status",
              #vrm_neigh3_mean_5_clip_01, 
...
for(i in seq(COMB.50m)){
  ...
  for(j in seq(ncol(tmp.50m))){
    k.50m <- k.50m + 1
    COMB2.50m[[k.50m]] <- formula(paste("M.hippocrepis.Abundance", "~", paste(tmp.50m[,j], collapse=" + ")))
  }
}
...
# Predict for test data
TestData.50m.M.hippocrepis.Abundance$Predicted.Species.Richness.50m <- predict.gam(ModelsResults.50m[[best.model.50m]], TestData.50m.M.hippocrepis.Abundance)
TestData.50m.M.hippocrepis.Abundance$Difference.Pred.True.50m <- abs((TestData.50m.M.hippocrepis.Abundance$M.hippocrepis.Abundance) - (TestData.50m.M.hippocrepis.Abundance$Predicted.Species.Richness.50m))
Mean.50m.Species.Richness.Predictive.Difference.50m <- mean(TestData.50m.M.hippocrepis.Abundance$Difference.Pred.True.50m)
...
TrainingData.75m.M.hippocrepis.Abundance <- Import.75m.Site.Summaries %>%
  filter(Training0.Test1 == "0") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
TestData.75m.M.hippocrepis.Abundance <- Import.75m.Site.Summaries %>%
  filter(Training0.Test1 == "1") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
ggcorr(TrainingData.75m.M.hippocrepis.Abundance[, 2:length(TrainingData.75m.M.hippocrepis.Abundance)], geom = "blank", label = TRUE, hjust = 0.8, label_size = 2, size = 3) +
  geom_point(size = 7, aes(color = coefficient > 0, alpha = abs(coefficient) > 0.5)) +
  scale_alpha_manual(values = c("TRUE" = 0.25, "FALSE" = 0)) +
  guides(color = FALSE, alpha = FALSE)
...
pairs(TrainingData.75m.M.hippocrepis.Abundance)

#Check Response Variable

plot(TrainingData.75m.M.hippocrepis.Abundance$M.hippocrepis.Abundance)


### Transformations
# Example: TrainingData.75m.M.hippocrepis.Abundance$sqrt.rugosity_mean_75_clip_01=sqrt(TrainingData.75m.M.hippocrepis.Abundance$rugosity_mean_75_clip_01)



# Models loop

data.75m <- TrainingData.75m.M.hippocrepis.Abundance

vars.75m <- c("M.hippocrepis.Abundance",
              #"Training0.Test1",
              "Year",
              "Status",
              #vrm_neigh3_mean_5_clip_01, 
...
for(i in seq(COMB.75m)){
  ...
  for(j in seq(ncol(tmp.75m))){
    k.75m <- k.75m + 1
    COMB2.75m[[k.75m]] <- formula(paste("M.hippocrepis.Abundance", "~", paste(tmp.75m[,j], collapse=" + ")))
  }
}
...
# Predict for test data
TestData.75m.M.hippocrepis.Abundance$Predicted.Species.Richness.75m <- predict.gam(ModelsResults.75m[[best.model.75m]], TestData.75m.M.hippocrepis.Abundance)
TestData.75m.M.hippocrepis.Abundance$Difference.Pred.True.75m <- abs((TestData.75m.M.hippocrepis.Abundance$M.hippocrepis.Abundance) - (TestData.75m.M.hippocrepis.Abundance$Predicted.Species.Richness.75m))
Mean.75m.Species.Richness.Predictive.Difference.75m <- mean(TestData.75m.M.hippocrepis.Abundance$Difference.Pred.True.75m)
...
TrainingData.100m.M.hippocrepis.Abundance <- Import.100m.Site.Summaries %>%
  filter(Training0.Test1 == "0") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
TestData.100m.M.hippocrepis.Abundance <- Import.100m.Site.Summaries %>%
  filter(Training0.Test1 == "1") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
ggcorr(TrainingData.100m.M.hippocrepis.Abundance[, 2:length(TrainingData.100m.M.hippocrepis.Abundance)], geom = "blank", label = TRUE, hjust = 0.8, label_size = 2, size = 3) +
  geom_point(size = 7, aes(color = coefficient > 0, alpha = abs(coefficient) > 0.5)) +
  scale_alpha_manual(values = c("TRUE" = 0.25, "FALSE" = 0)) +
  guides(color = FALSE, alpha = FALSE)
...
pairs(TrainingData.100m.M.hippocrepis.Abundance)

#Check Response Variable

plot(TrainingData.100m.M.hippocrepis.Abundance$M.hippocrepis.Abundance)


### Transformations
# Example: TrainingData.100m.M.hippocrepis.Abundance$sqrt.rugosity_mean_100_clip_01=sqrt(TrainingData.100m.M.hippocrepis.Abundance$rugosity_mean_100_clip_01)



# Models loop

data.100m <- TrainingData.100m.M.hippocrepis.Abundance

vars.100m <- c("M.hippocrepis.Abundance",
               #"Training0.Test1",
               "Year",
               "Status",
               #vrm_neigh3_mean_5_clip_01, 
...
for(i in seq(COMB.100m)){
  ...
  for(j in seq(ncol(tmp.100m))){
    k.100m <- k.100m + 1
    COMB2.100m[[k.100m]] <- formula(paste("M.hippocrepis.Abundance", "~", paste(tmp.100m[,j], collapse=" + ")))
  }
}
...
# Predict for test data
TestData.100m.M.hippocrepis.Abundance$Predicted.Species.Richness.100m <- predict.gam(ModelsResults.100m[[best.model.100m]], TestData.100m.M.hippocrepis.Abundance)
TestData.100m.M.hippocrepis.Abundance$Difference.Pred.True.100m <- abs((TestData.100m.M.hippocrepis.Abundance$M.hippocrepis.Abundance) - (TestData.100m.M.hippocrepis.Abundance$Predicted.Species.Richness.100m))
Mean.100m.Species.Richness.Predictive.Difference.100m <- mean(TestData.100m.M.hippocrepis.Abundance$Difference.Pred.True.100m)
...
TrainingData.150m.M.hippocrepis.Abundance <- Import.150m.Site.Summaries %>%
  filter(Training0.Test1 == "0") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
TestData.150m.M.hippocrepis.Abundance <- Import.150m.Site.Summaries %>%
  filter(Training0.Test1 == "1") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
ggcorr(TrainingData.150m.M.hippocrepis.Abundance[, 2:length(TrainingData.150m.M.hippocrepis.Abundance)], geom = "blank", label = TRUE, hjust = 0.8, label_size = 2, size = 3) +
  geom_point(size = 7, aes(color = coefficient > 0, alpha = abs(coefficient) > 0.5)) +
  scale_alpha_manual(values = c("TRUE" = 0.25, "FALSE" = 0)) +
  guides(color = FALSE, alpha = FALSE)
...
pairs(TrainingData.150m.M.hippocrepis.Abundance)

#Check Response Variable

plot(TrainingData.150m.M.hippocrepis.Abundance$M.hippocrepis.Abundance)


### Transformations
# Example: TrainingData.150m.M.hippocrepis.Abundance$sqrt.rugosity_mean_150_clip_01=sqrt(TrainingData.150m.M.hippocrepis.Abundance$rugosity_mean_150_clip_01)



# Models loop

data.150m <- TrainingData.150m.M.hippocrepis.Abundance

vars.150m <- c("M.hippocrepis.Abundance",
               #"Training0.Test1",
               "Year",
               "Status",
               #vrm_neigh3_mean_5_clip_01, 
...
for(i in seq(COMB.150m)){
  ...
  for(j in seq(ncol(tmp.150m))){
    k.150m <- k.150m + 1
    COMB2.150m[[k.150m]] <- formula(paste("M.hippocrepis.Abundance", "~", paste(tmp.150m[,j], collapse=" + ")))
  }
}
...
# Predict for test data
TestData.150m.M.hippocrepis.Abundance$Predicted.Species.Richness.150m <- predict.gam(ModelsResults.150m[[best.model.150m]], TestData.150m.M.hippocrepis.Abundance)
TestData.150m.M.hippocrepis.Abundance$Difference.Pred.True.150m <- abs((TestData.150m.M.hippocrepis.Abundance$M.hippocrepis.Abundance) - (TestData.150m.M.hippocrepis.Abundance$Predicted.Species.Richness.150m))
Mean.150m.Species.Richness.Predictive.Difference.150m <- mean(TestData.150m.M.hippocrepis.Abundance$Difference.Pred.True.150m)
...
TrainingData.200m.M.hippocrepis.Abundance <- Import.200m.Site.Summaries %>%
  filter(Training0.Test1 == "0") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
TestData.200m.M.hippocrepis.Abundance <- Import.200m.Site.Summaries %>%
  filter(Training0.Test1 == "1") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
ggcorr(TrainingData.200m.M.hippocrepis.Abundance[, 2:length(TrainingData.200m.M.hippocrepis.Abundance)], geom = "blank", label = TRUE, hjust = 0.8, label_size = 2, size = 3) +
  geom_point(size = 7, aes(color = coefficient > 0, alpha = abs(coefficient) > 0.5)) +
  scale_alpha_manual(values = c("TRUE" = 0.25, "FALSE" = 0)) +
  guides(color = FALSE, alpha = FALSE)
...
pairs(TrainingData.200m.M.hippocrepis.Abundance)

#Check Response Variable

plot(TrainingData.200m.M.hippocrepis.Abundance$M.hippocrepis.Abundance)


### Transformations
# Example: TrainingData.200m.M.hippocrepis.Abundance$sqrt.rugosity_mean_200_clip_01=sqrt(TrainingData.200m.M.hippocrepis.Abundance$rugosity_mean_200_clip_01)



# Models loop

data.200m <- TrainingData.200m.M.hippocrepis.Abundance

vars.200m <- c("M.hippocrepis.Abundance",
               #"Training0.Test1",
               "Year",
               "Status",
               #vrm_neigh3_mean_5_clip_01, 
...
for(i in seq(COMB.200m)){
  ...
  for(j in seq(ncol(tmp.200m))){
    k.200m <- k.200m + 1
    COMB2.200m[[k.200m]] <- formula(paste("M.hippocrepis.Abundance", "~", paste(tmp.200m[,j], collapse=" + ")))
  }
}
...
# Predict for test data
TestData.200m.M.hippocrepis.Abundance$Predicted.Species.Richness.200m <- predict.gam(ModelsResults.200m[[best.model.200m]], TestData.200m.M.hippocrepis.Abundance)
TestData.200m.M.hippocrepis.Abundance$Difference.Pred.True.200m <- abs((TestData.200m.M.hippocrepis.Abundance$M.hippocrepis.Abundance) - (TestData.200m.M.hippocrepis.Abundance$Predicted.Species.Richness.200m))
Mean.200m.Species.Richness.Predictive.Difference.200m <- mean(TestData.200m.M.hippocrepis.Abundance$Difference.Pred.True.200m)
...
TrainingData.250m.M.hippocrepis.Abundance <- Import.250m.Site.Summaries %>%
  filter(Training0.Test1 == "0") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
TestData.250m.M.hippocrepis.Abundance <- Import.250m.Site.Summaries %>%
  filter(Training0.Test1 == "1") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
ggcorr(TrainingData.250m.M.hippocrepis.Abundance[, 2:length(TrainingData.250m.M.hippocrepis.Abundance)], geom = "blank", label = TRUE, hjust = 0.8, label_size = 2, size = 3) +
  geom_point(size = 7, aes(color = coefficient > 0, alpha = abs(coefficient) > 0.5)) +
  scale_alpha_manual(values = c("TRUE" = 0.25, "FALSE" = 0)) +
  guides(color = FALSE, alpha = FALSE)
...
pairs(TrainingData.250m.M.hippocrepis.Abundance)

#Check Response Variable

plot(TrainingData.250m.M.hippocrepis.Abundance$M.hippocrepis.Abundance)


### Transformations
# Example: TrainingData.250m.M.hippocrepis.Abundance$sqrt.rugosity_mean_250_clip_01=sqrt(TrainingData.250m.M.hippocrepis.Abundance$rugosity_mean_250_clip_01)



# Models loop

data.250m <- TrainingData.250m.M.hippocrepis.Abundance

vars.250m <- c("M.hippocrepis.Abundance",
               #"Training0.Test1",
               "Year",
               "Status",
               #vrm_neigh3_mean_5_clip_01, 
...
for(i in seq(COMB.250m)){
  ...
  for(j in seq(ncol(tmp.250m))){
    k.250m <- k.250m + 1
    COMB2.250m[[k.250m]] <- formula(paste("M.hippocrepis.Abundance", "~", paste(tmp.250m[,j], collapse=" + ")))
  }
}
...
# Predict for test data
TestData.250m.M.hippocrepis.Abundance$Predicted.Species.Richness.250m <- predict.gam(ModelsResults.250m[[best.model.250m]], TestData.250m.M.hippocrepis.Abundance)
TestData.250m.M.hippocrepis.Abundance$Difference.Pred.True.250m <- abs((TestData.250m.M.hippocrepis.Abundance$M.hippocrepis.Abundance) - (TestData.250m.M.hippocrepis.Abundance$Predicted.Species.Richness.250m))
Mean.250m.Species.Richness.Predictive.Difference.250m <- mean(TestData.250m.M.hippocrepis.Abundance$Difference.Pred.True.250m)
...
TrainingData.300m.M.hippocrepis.Abundance <- Import.300m.Site.Summaries %>%
  filter(Training0.Test1 == "0") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
TestData.300m.M.hippocrepis.Abundance <- Import.300m.Site.Summaries %>%
  filter(Training0.Test1 == "1") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
ggcorr(TrainingData.300m.M.hippocrepis.Abundance[, 2:length(TrainingData.300m.M.hippocrepis.Abundance)], geom = "blank", label = TRUE, hjust = 0.8, label_size = 2, size = 3) +
  geom_point(size = 7, aes(color = coefficient > 0, alpha = abs(coefficient) > 0.5)) +
  scale_alpha_manual(values = c("TRUE" = 0.25, "FALSE" = 0)) +
  guides(color = FALSE, alpha = FALSE)
...
pairs(TrainingData.300m.M.hippocrepis.Abundance)

#Check Response Variable

plot(TrainingData.300m.M.hippocrepis.Abundance$M.hippocrepis.Abundance)


### Transformations
# Example: TrainingData.300m.M.hippocrepis.Abundance$sqrt.rugosity_mean_300_clip_01=sqrt(TrainingData.300m.M.hippocrepis.Abundance$rugosity_mean_300_clip_01)



# Models loop

data.300m <- TrainingData.300m.M.hippocrepis.Abundance

vars.300m <- c("M.hippocrepis.Abundance",
               #"Training0.Test1",
               "Year",
               "Status",
               #vrm_neigh3_mean_5_clip_01, 
...
for(i in seq(COMB.300m)){
  ...
  for(j in seq(ncol(tmp.300m))){
    k.300m <- k.300m + 1
    COMB2.300m[[k.300m]] <- formula(paste("M.hippocrepis.Abundance", "~", paste(tmp.300m[,j], collapse=" + ")))
  }
}
...
# Predict for test data
TestData.300m.M.hippocrepis.Abundance$Predicted.Species.Richness.300m <- predict.gam(ModelsResults.300m[[best.model.300m]], TestData.300m.M.hippocrepis.Abundance)
TestData.300m.M.hippocrepis.Abundance$Difference.Pred.True.300m <- abs((TestData.300m.M.hippocrepis.Abundance$M.hippocrepis.Abundance) - (TestData.300m.M.hippocrepis.Abundance$Predicted.Species.Richness.300m))
Mean.300m.Species.Richness.Predictive.Difference.300m <- mean(TestData.300m.M.hippocrepis.Abundance$Difference.Pred.True.300m)
...
TrainingData.400m.M.hippocrepis.Abundance <- Import.400m.Site.Summaries %>%
  filter(Training0.Test1 == "0") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
TestData.400m.M.hippocrepis.Abundance <- Import.400m.Site.Summaries %>%
  filter(Training0.Test1 == "1") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
ggcorr(TrainingData.400m.M.hippocrepis.Abundance[, 2:length(TrainingData.400m.M.hippocrepis.Abundance)], geom = "blank", label = TRUE, hjust = 0.8, label_size = 2, size = 3) +
  geom_point(size = 7, aes(color = coefficient > 0, alpha = abs(coefficient) > 0.5)) +
  scale_alpha_manual(values = c("TRUE" = 0.25, "FALSE" = 0)) +
  guides(color = FALSE, alpha = FALSE)
...
pairs(TrainingData.400m.M.hippocrepis.Abundance)

#Check Response Variable

plot(TrainingData.400m.M.hippocrepis.Abundance$M.hippocrepis.Abundance)


### Transformations
# Example: TrainingData.400m.M.hippocrepis.Abundance$sqrt.rugosity_mean_400_clip_01=sqrt(TrainingData.400m.M.hippocrepis.Abundance$rugosity_mean_400_clip_01)



# Models loop

data.400m <- TrainingData.400m.M.hippocrepis.Abundance

vars.400m <- c("M.hippocrepis.Abundance",
               #"Training0.Test1",
               "Year",
               "Status",
               #vrm_neigh3_mean_5_clip_01, 
...
for(i in seq(COMB.400m)){
  ...
  for(j in seq(ncol(tmp.400m))){
    k.400m <- k.400m + 1
    COMB2.400m[[k.400m]] <- formula(paste("M.hippocrepis.Abundance", "~", paste(tmp.400m[,j], collapse=" + ")))
  }
}
...
# Predict for test data
TestData.400m.M.hippocrepis.Abundance$Predicted.Species.Richness.400m <- predict.gam(ModelsResults.400m[[best.model.400m]], TestData.400m.M.hippocrepis.Abundance)
TestData.400m.M.hippocrepis.Abundance$Difference.Pred.True.400m <- abs((TestData.400m.M.hippocrepis.Abundance$M.hippocrepis.Abundance) - (TestData.400m.M.hippocrepis.Abundance$Predicted.Species.Richness.400m))
Mean.400m.Species.Richness.Predictive.Difference.400m <- mean(TestData.400m.M.hippocrepis.Abundance$Difference.Pred.True.400m)
...
TrainingData.500m.M.hippocrepis.Abundance <- Import.500m.Site.Summaries %>%
  filter(Training0.Test1 == "0") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
TestData.500m.M.hippocrepis.Abundance <- Import.500m.Site.Summaries %>%
  filter(Training0.Test1 == "1") %>%
  dplyr::select(M.hippocrepis.Abundance,
                #Training0.Test1,
                Year,
...
ggcorr(TrainingData.500m.M.hippocrepis.Abundance[, 2:length(TrainingData.500m.M.hippocrepis.Abundance)], geom = "blank", label = TRUE, hjust = 0.8, label_size = 2, size = 3) +
  geom_point(size = 7, aes(color = coefficient > 0, alpha = abs(coefficient) > 0.5)) +
  scale_alpha_manual(values = c("TRUE" = 0.25, "FALSE" = 0)) +
  guides(color = FALSE, alpha = FALSE)
...
pairs(TrainingData.500m.M.hippocrepis.Abundance)

#Check Response Variable

plot(TrainingData.500m.M.hippocrepis.Abundance$M.hippocrepis.Abundance)


### Transformations
# Example: TrainingData.500m.M.hippocrepis.Abundance$sqrt.rugosity_mean_500_clip_01=sqrt(TrainingData.500m.M.hippocrepis.Abundance$rugosity_mean_500_clip_01)



# Models loop

data.500m <- TrainingData.500m.M.hippocrepis.Abundance

vars.500m <- c("M.hippocrepis.Abundance",
               #"Training0.Test1",
               "Year",
               "Status",
               #vrm_neigh3_mean_5_clip_01, 
...
for(i in seq(COMB.500m)){
  ...
  for(j in seq(ncol(tmp.500m))){
    k.500m <- k.500m + 1
    COMB2.500m[[k.500m]] <- formula(paste("M.hippocrepis.Abundance", "~", paste(tmp.500m[,j], collapse=" + ")))
  }
}
...
# Predict for test data
TestData.500m.M.hippocrepis.Abundance$Predicted.Species.Richness.500m <- predict.gam(ModelsResults.500m[[best.model.500m]], TestData.500m.M.hippocrepis.Abundance)
TestData.500m.M.hippocrepis.Abundance$Difference.Pred.True.500m <- abs((TestData.500m.M.hippocrepis.Abundance$M.hippocrepis.Abundance) - (TestData.500m.M.hippocrepis.Abundance$Predicted.Species.Richness.500m))
Mean.500m.Species.Richness.Predictive.Difference.500m <- mean(TestData.500m.M.hippocrepis.Abundance$Difference.Pred.True.500m)
...
BestModelSelection$Pred.Obs.Correlation <- c(with(TestData.5m.M.hippocrepis.Abundance, cor(M.hippocrepis.Abundance, Predicted.Species.Richness.5m, use = "complete.obs")),
                                             with(TestData.10m.M.hippocrepis.Abundance, cor(M.hippocrepis.Abundance, Predicted.Species.Richness.10m, use = "complete.obs")),
                                             with(TestData.25m.M.hippocrepis.Abundance, cor(M.hippocrepis.Abundance, Predicted.Species.Richness.25m, use = "complete.obs")),
                                             with(TestData.50m.M.hippocrepis.Abundance, cor(M.hippocrepis.Abundance, Predicted.Species.Richness.50m, use = "complete.obs")),
                                             with(TestData.75m.M.hippocrepis.Abundance, cor(M.hippocrepis.Abundance, Predicted.Species.Richness.75m, use = "complete.obs")),
                                             with(TestData.100m.M.hippocrepis.Abundance, cor(M.hippocrepis.Abundance, Predicted.Species.Richness.100m, use = "complete.obs")),
                                             with(TestData.150m.M.hippocrepis.Abundance, cor(M.hippocrepis.Abundance, Predicted.Species.Richness.150m, use = "complete.obs")),
                                             with(TestData.200m.M.hippocrepis.Abundance, cor(M.hippocrepis.Abundance, Predicted.Species.Richness.200m, use = "complete.obs")),
                                             with(TestData.250m.M.hippocrepis.Abundance, cor(M.hippocrepis.Abundance, Predicted.Species.Richness.250m, use = "complete.obs")),
                                             with(TestData.300m.M.hippocrepis.Abundance, cor(M.hippocrepis.Abundance, Predicted.Species.Richness.300m, use = "complete.obs")),
                                             with(TestData.400m.M.hippocrepis.Abundance, cor(M.hippocrepis.Abundance, Predicted.Species.Richness.400m, use = "complete.obs")),
                                             with(TestData.500m.M.hippocrepis.Abundance, cor(M.hippocrepis.Abundance, Predicted.Species.Richness.500m, use = "complete.obs"))
)
...
write.table(BestModelSelection, file = "01_BestModelSelection_M.hippocrepis.Abundance.txt" , sep = "\t", row.names = FALSE)
...
for(i in seq(COMB.VI)){
  ...
  for(j in seq(ncol(tmp.VI))){
    k.VI <- k.VI + 1
    COMB2.VI[[k.VI]] <- formula(paste("M.hippocrepis.Abundance", "~", paste(tmp.VI[,j], collapse=" + ")))
  }
}
...
output.VI$Subset <- "M.hippocrepis.Abundance"

VariableImportance <- output.VI %>%
  dplyr::select("BestModel.Variables", "Subset", "VariableImportance")

write.table(VariableImportance, file = "01_VariableImportance_M.hippocrepis.Abundance.txt" , sep = "\t", row.names = FALSE)
...
# Make GAM Plots
pdf(file = "M.hippocrepis.Abundance_Best_Plots.pdf")
plot.gam(BestModel)
dev.off()
...
M.hippocrepis.Abundance_AICc_01 <- ggplot(data=BestModelSelection, aes(x=BestModelSelection$Scale, y=BestModelSelection$AICc, group=1)) +
  geom_line()+
  geom_point()+
...
M.hippocrepis.Abundance_AICc_01
ggsave("AICc_M.hippocrepis.Abundance_01.png", plot = last_plot(), device = "png",
       scale = 1, dpi = 300, limitsize = TRUE)


M.hippocrepis.Abundance_Weight_01 <- ggplot(data=BestModelSelection, aes(x=BestModelSelection$Scale, y=BestModelSelection$weight, group=1)) +
  geom_line()+
  geom_point()+
...
M.hippocrepis.Abundance_Weight_01
ggsave("Weight_M.hippocrepis.Abundance_01.png", plot = last_plot(), device = "png",
       scale = 1, dpi = 300, limitsize = TRUE)


M.hippocrepis.Abundance_Pred.Obs.Correlation_01 <- ggplot(data=BestModelSelection, aes(x=BestModelSelection$Scale, y=BestModelSelection$Pred.Obs.Correlation, group=1)) +
  geom_line()+
  geom_point()+
...
M.hippocrepis.Abundance_Pred.Obs.Correlation_01
ggsave("Pred.Obs.Correlation_M.hippocrepis.Abundance_01.png", plot = last_plot(), device = "png",
       scale = 1, dpi = 300, limitsize = TRUE)


M.hippocrepis.Abundance_logLik_01 <- ggplot(data=BestModelSelection, aes(x=BestModelSelection$Scale, y=BestModelSelection$logLik, group=1)) +
  geom_line()+
  geom_point()+
...
M.hippocrepis.Abundance_logLik_01
ggsave("logLik_M.hippocrepis.Abundance_01.png", plot = last_plot(), device = "png",
       scale = 1, dpi = 300, limitsize = TRUE)


M.hippocrepis.Abundance_AIC.mgcv_01 <- ggplot(data=BestModelSelection, aes(x=BestModelSelection$Scale, y=BestModelSelection$AIC.mgcv, group=1)) +
  geom_line()+
  geom_point()+
...
M.hippocrepis.Abundance_AIC.mgcv_01
ggsave("AICmgcv_M.hippocrepis.Abundance_01.png", plot = last_plot(), device = "png",
       scale = 1, dpi = 300, limitsize = TRUE)


M.hippocrepis.Abundance_DevExplained_01 <- ggplot(data=BestModelSelection, aes(x=BestModelSelection$Scale, y=BestModelSelection$Deviance.Explained, group=1)) +
  geom_line()+
  geom_point()+
...
M.hippocrepis.Abundance_DevExplained_01
ggsave("DevExplained_M.hippocrepis.Abundance_01.png", plot = last_plot(), device = "png",
       scale = 1, dpi = 300, limitsize = TRUE)
