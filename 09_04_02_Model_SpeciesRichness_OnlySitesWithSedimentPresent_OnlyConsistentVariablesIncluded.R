...
# This script is for modelling species richness of sites with sediment present
...
# Import Files
Import.5m.Site.Summaries <- read.table("01_5m_SiteSummariesSEDIMENT.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE) 
Import.10m.Site.Summaries <- read.table("01_10m_SiteSummariesSEDIMENT.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE) 
Import.25m.Site.Summaries <- read.table("01_25m_SiteSummariesSEDIMENT.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE) 
Import.50m.Site.Summaries <- read.table("01_50m_SiteSummariesSEDIMENT.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE) 
Import.75m.Site.Summaries <- read.table("01_75m_SiteSummariesSEDIMENT.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE) 
Import.100m.Site.Summaries <- read.table("01_100m_SiteSummariesSEDIMENT.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE) 
Import.150m.Site.Summaries <- read.table("01_150m_SiteSummariesSEDIMENT.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE) 
Import.200m.Site.Summaries <- read.table("01_200m_SiteSummariesSEDIMENT.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE) 
Import.250m.Site.Summaries <- read.table("01_250m_SiteSummariesSEDIMENT.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE) 
Import.300m.Site.Summaries <- read.table("01_300m_SiteSummariesSEDIMENT.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE) 
Import.400m.Site.Summaries <- read.table("01_400m_SiteSummariesSEDIMENT.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE) 
Import.500m.Site.Summaries <- read.table("01_500m_SiteSummariesSEDIMENT.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE) 
...
Cons_5m_SpRichness_SEDIMENT <- gam(Formula.5m, data = data.5m)

# Predict for test data
TestData.5m.SpeciesRichness$Predicted.Species.Richness.5m <- as.numeric(predict.gam(Cons_5m_SpRichness_SEDIMENT, TestData.5m.SpeciesRichness))
...
Cons_10m_SpRichness_SEDIMENT <- gam(Formula.10m, data = data.10m)

# Predict for test data
TestData.10m.SpeciesRichness$Predicted.Species.Richness.10m <- as.numeric(predict.gam(Cons_10m_SpRichness_SEDIMENT, TestData.10m.SpeciesRichness))
...
Cons_25m_SpRichness_SEDIMENT <- gam(Formula.25m, data = data.25m)

# Predict for test data
TestData.25m.SpeciesRichness$Predicted.Species.Richness.25m <- as.numeric(predict.gam(Cons_25m_SpRichness_SEDIMENT, TestData.25m.SpeciesRichness))
...
Cons_50m_SpRichness_SEDIMENT <- gam(Formula.50m, data = data.50m)

# Predict for test data
TestData.50m.SpeciesRichness$Predicted.Species.Richness.50m <- as.numeric(predict.gam(Cons_50m_SpRichness_SEDIMENT, TestData.50m.SpeciesRichness))
...
Cons_75m_SpRichness_SEDIMENT <- gam(Formula.75m, data = data.75m)

# Predict for test data
TestData.75m.SpeciesRichness$Predicted.Species.Richness.75m <- as.numeric(predict.gam(Cons_75m_SpRichness_SEDIMENT, TestData.75m.SpeciesRichness))
...
Cons_100m_SpRichness_SEDIMENT <- gam(Formula.100m, data = data.100m)

# Predict for test data
TestData.100m.SpeciesRichness$Predicted.Species.Richness.100m <- as.numeric(predict.gam(Cons_100m_SpRichness_SEDIMENT, TestData.100m.SpeciesRichness))
...
Cons_150m_SpRichness_SEDIMENT <- gam(Formula.150m, data = data.150m)

# Predict for test data
TestData.150m.SpeciesRichness$Predicted.Species.Richness.150m <- as.numeric(predict.gam(Cons_150m_SpRichness_SEDIMENT, TestData.150m.SpeciesRichness))
...
Cons_200m_SpRichness_SEDIMENT <- gam(Formula.200m, data = data.200m)

# Predict for test data
TestData.200m.SpeciesRichness$Predicted.Species.Richness.200m <- as.numeric(predict.gam(Cons_200m_SpRichness_SEDIMENT, TestData.200m.SpeciesRichness))
...
Cons_250m_SpRichness_SEDIMENT <- gam(Formula.250m, data = data.250m)

# Predict for test data
TestData.250m.SpeciesRichness$Predicted.Species.Richness.250m <- as.numeric(predict.gam(Cons_250m_SpRichness_SEDIMENT, TestData.250m.SpeciesRichness))
...
Cons_300m_SpRichness_SEDIMENT <- gam(Formula.300m, data = data.300m)

# Predict for test data
TestData.300m.SpeciesRichness$Predicted.Species.Richness.300m <- as.numeric(predict.gam(Cons_300m_SpRichness_SEDIMENT, TestData.300m.SpeciesRichness))
...
Cons_400m_SpRichness_SEDIMENT <- gam(Formula.400m, data = data.400m)

# Predict for test data
TestData.400m.SpeciesRichness$Predicted.Species.Richness.400m <- as.numeric(predict.gam(Cons_400m_SpRichness_SEDIMENT, TestData.400m.SpeciesRichness))
...
Cons_500m_SpRichness_SEDIMENT <- gam(Formula.500m, data = data.500m)

# Predict for test data
TestData.500m.SpeciesRichness$Predicted.Species.Richness.500m <- as.numeric(predict.gam(Cons_500m_SpRichness_SEDIMENT, TestData.500m.SpeciesRichness))
...
best.model.selection.5m$Formula <- as.character(c(Cons_5m_SpRichness_SEDIMENT$formula))
best.model.selection.10m$Formula <- as.character(c(Cons_10m_SpRichness_SEDIMENT$formula))
best.model.selection.25m$Formula <- as.character(c(Cons_25m_SpRichness_SEDIMENT$formula))
best.model.selection.50m$Formula <- as.character(c(Cons_50m_SpRichness_SEDIMENT$formula))
best.model.selection.75m$Formula <- as.character(c(Cons_75m_SpRichness_SEDIMENT$formula))
best.model.selection.100m$Formula <- as.character(c(Cons_100m_SpRichness_SEDIMENT$formula))
best.model.selection.150m$Formula <- as.character(c(Cons_150m_SpRichness_SEDIMENT$formula))
best.model.selection.200m$Formula <- as.character(c(Cons_200m_SpRichness_SEDIMENT$formula))
best.model.selection.250m$Formula <- as.character(c(Cons_250m_SpRichness_SEDIMENT$formula))
best.model.selection.300m$Formula <- as.character(c(Cons_300m_SpRichness_SEDIMENT$formula))
best.model.selection.400m$Formula <- as.character(c(Cons_400m_SpRichness_SEDIMENT$formula))
best.model.selection.500m$Formula <- as.character(c(Cons_500m_SpRichness_SEDIMENT$formula))

best.model.selection.5m$AIC.mgcv <- as.character(c(Cons_5m_SpRichness_SEDIMENT$aic))
best.model.selection.10m$AIC.mgcv <- as.character(c(Cons_10m_SpRichness_SEDIMENT$aic))
best.model.selection.25m$AIC.mgcv <- as.character(c(Cons_25m_SpRichness_SEDIMENT$aic))
best.model.selection.50m$AIC.mgcv <- as.character(c(Cons_50m_SpRichness_SEDIMENT$aic))
best.model.selection.75m$AIC.mgcv <- as.character(c(Cons_75m_SpRichness_SEDIMENT$aic))
best.model.selection.100m$AIC.mgcv <- as.character(c(Cons_100m_SpRichness_SEDIMENT$aic))
best.model.selection.150m$AIC.mgcv <- as.character(c(Cons_150m_SpRichness_SEDIMENT$aic))
best.model.selection.200m$AIC.mgcv <- as.character(c(Cons_200m_SpRichness_SEDIMENT$aic))
best.model.selection.250m$AIC.mgcv <- as.character(c(Cons_250m_SpRichness_SEDIMENT$aic))
best.model.selection.300m$AIC.mgcv <- as.character(c(Cons_300m_SpRichness_SEDIMENT$aic))
best.model.selection.400m$AIC.mgcv <- as.character(c(Cons_400m_SpRichness_SEDIMENT$aic))
best.model.selection.500m$AIC.mgcv <- as.character(c(Cons_500m_SpRichness_SEDIMENT$aic))

best.model.selection.5m$Deviance.Explained <- as.numeric(((Cons_5m_SpRichness_SEDIMENT$null.deviance - Cons_5m_SpRichness_SEDIMENT$deviance)/Cons_5m_SpRichness_SEDIMENT$null.deviance)*100)
best.model.selection.10m$Deviance.Explained <- as.numeric(((Cons_10m_SpRichness_SEDIMENT$null.deviance - Cons_10m_SpRichness_SEDIMENT$deviance)/Cons_10m_SpRichness_SEDIMENT$null.deviance)*100)
best.model.selection.25m$Deviance.Explained <- as.numeric(((Cons_25m_SpRichness_SEDIMENT$null.deviance - Cons_25m_SpRichness_SEDIMENT$deviance)/Cons_25m_SpRichness_SEDIMENT$null.deviance)*100)
best.model.selection.50m$Deviance.Explained <- as.numeric(((Cons_50m_SpRichness_SEDIMENT$null.deviance - Cons_50m_SpRichness_SEDIMENT$deviance)/Cons_50m_SpRichness_SEDIMENT$null.deviance)*100)
best.model.selection.75m$Deviance.Explained <- as.numeric(((Cons_75m_SpRichness_SEDIMENT$null.deviance - Cons_75m_SpRichness_SEDIMENT$deviance)/Cons_75m_SpRichness_SEDIMENT$null.deviance)*100)
best.model.selection.100m$Deviance.Explained <- as.numeric(((Cons_100m_SpRichness_SEDIMENT$null.deviance - Cons_100m_SpRichness_SEDIMENT$deviance)/Cons_100m_SpRichness_SEDIMENT$null.deviance)*100)
best.model.selection.150m$Deviance.Explained <- as.numeric(((Cons_150m_SpRichness_SEDIMENT$null.deviance - Cons_150m_SpRichness_SEDIMENT$deviance)/Cons_150m_SpRichness_SEDIMENT$null.deviance)*100)
best.model.selection.200m$Deviance.Explained <- as.numeric(((Cons_200m_SpRichness_SEDIMENT$null.deviance - Cons_200m_SpRichness_SEDIMENT$deviance)/Cons_200m_SpRichness_SEDIMENT$null.deviance)*100)
best.model.selection.250m$Deviance.Explained <- as.numeric(((Cons_250m_SpRichness_SEDIMENT$null.deviance - Cons_250m_SpRichness_SEDIMENT$deviance)/Cons_250m_SpRichness_SEDIMENT$null.deviance)*100)
best.model.selection.300m$Deviance.Explained <- as.numeric(((Cons_300m_SpRichness_SEDIMENT$null.deviance - Cons_300m_SpRichness_SEDIMENT$deviance)/Cons_300m_SpRichness_SEDIMENT$null.deviance)*100)
best.model.selection.400m$Deviance.Explained <- as.numeric(((Cons_400m_SpRichness_SEDIMENT$null.deviance - Cons_400m_SpRichness_SEDIMENT$deviance)/Cons_400m_SpRichness_SEDIMENT$null.deviance)*100)
best.model.selection.500m$Deviance.Explained <- as.numeric(((Cons_500m_SpRichness_SEDIMENT$null.deviance - Cons_500m_SpRichness_SEDIMENT$deviance)/Cons_500m_SpRichness_SEDIMENT$null.deviance)*100)
